...
function createReactElements(nodes, tokens, createElement) {
  ...
  for (let token of tokens) {
    ...
    switch(token.type) {
      ...
      case 'softbreak':
      case 'hr':
        lastNode().children.push(createElement(token.tag, {key}))
        break
      case 'code_block':
      case 'code_inline':
      case 'fence':
        lastNode().children.push(createElement(token.tag, {key}, token.content))
        break
      case 'image':
      {
        const props = Object.assign({key}, parseAttrs(token.attrs), {alt: token.content})
        lastNode().children.push(createElement(token.tag, props))
        break
      }
      case 'heading_close':
      ...
      case 's_close':
      {
        ...
        const element = createElement(node.type, props, node.children.slice())
...
function createReactTree(tokens, type, createElement) {
  ...
  return createElement(root.type, root.props, root.children)
}
